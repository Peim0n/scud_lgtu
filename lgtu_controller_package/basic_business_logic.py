...
def grant_access(engine, reader: str = "w1", duration: float = 2.0) -> None:
    ...
    time.sleep(duration)
    set_shift_pins(engine, {f"{reader}_green": False, "rel2": False})

def deny_access(engine, reader: str = "w1", beep_count: int = 2) -> None:
    """Отказать в доступе: красный индикатор + пищалка."""
    set_shift_pins(engine, {f"{reader}_red": True})
    beep_sequence(engine, beep_count)
    time.sleep(1.0)
