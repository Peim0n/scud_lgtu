...
class PassageDetector:
    ...
    inner_addr : int
        Адрес мультиплексора внутреннего датчика (0-7).
    outer_addr : int
        Адрес мультиплексора внешнего датчика (0-7).
    ...
    def __init__(
        ...
        self._inner_addr = inner_addr
        self._outer_addr = outer_addr
    ...
    def on_mux_state(self, states: dict, timestamp: float) -> None:
        ...
        ----------
        states : dict
            Словарь состояний адресов мультиплексора.
            Ключи зависят от формата MuxWorker (строковое представление dict).
            Значения — 0 или 1.
        timestamp : float
            Время получения состояния.
        """
        inner_val = self._get_value(states, self._inner_addr)
        outer_val = self._get_value(states, self._outer_addr)

        with self._lock:
            self._update_sensor("inner", self._inner, inner_val, timestamp)
            self._update_sensor("outer", self._outer, outer_val, timestamp)

    def _get_value(self, states: dict, addr: int) -> int:
        """Извлечь значение адреса из состояния мультиплексора."""
        # MuxWorker использует ключи вида "{'PA6': 0, 'PA11': 0, 'PA12': 0}"
        # для каждой комбинации адресных пинов.
        # Нам нужно найти ключ, в котором адресные пины соответствуют addr.
        for key, val in states.items():
            # Пытаемся распарсить строку как dict
            try:
                d = eval(key)  # безопасно, т.к. ключ контролируется MuxWorker
            except Exception:
                continue
            # addr — это номер от 0 до 7, бит A0 — младший
            # Формируем ожидаемый словарь и сравниваем
            expected = self._addr_to_dict(addr)
            if d == expected:
                return int(val)
        return 0

    def _addr_to_dict(self, addr: int) -> dict:
        """Преобразовать номер адреса в словарь состояний адресных пинов."""
        # Порядок пинов в MuxWorker: A0 — младший бит
        # Ключ формируется из sorted(addr_pins.keys())
        # В config.yml: A0, A1, A2 → PA6, PA11, PA12
        # Следовательно, в dict ключи идут в порядке A0, A1, A2
        return {
            "A0": (addr >> 0) & 1,
            "A1": (addr >> 1) & 1,
            "A2": (addr >> 2) & 1,
            ...
            self._event_queue.put_nowait(
                ScudEvent(
                    ...
                    payload={
                        ...
                        "direction": direction,
                        "duration": duration,
                        "inner_addr": self._inner_addr,
                        "outer_addr": self._outer_addr,
                    },
                )
            )
        except Full:
