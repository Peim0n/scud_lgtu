...
турникет, при отжатии (state=True) запускаем таймер закрытия. Обработчик использует
конфигурацию устройств для динамического определения действия кнопки (open_entry,
open_exit, close).
