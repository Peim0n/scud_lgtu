"""
Шина событий для слоя приложения системы СКУД.

Этот модуль реализует паттерн Publisher-Subscriber для асинхронной обработки событий.
Поддерживает как синхронные, так и асинхронные обработчики. Во время тревоги игнорирует
все события кроме PassageDetected (датчики) для точного подсчёта людей при эвакуации.

Классы
-------
- EventBus: шина событий для публикации и подписки на события

Методы EventBus
---------------
- subscribe: подписаться на тип события
- unsubscribe: отписаться от типа события
- publish: опубликовать событие всем подписчикам
- set_event_loop: установить event loop для асинхронных обработчиков
"""
import asyncio
import threading
