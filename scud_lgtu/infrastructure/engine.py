"""
Главный оркестратор ScudEngine системы СКУД.

Этот модуль реализует главный движок, который запускает все hardware-модули как потоки
и предоставляет бизнес-логике интерфейсы для управления оборудованием. Движок координирует
работу GPIO-контроллера, считывателей Wiegand, Serial-портов, мультиплексора и других
компонентов, обеспечивая централизованную обработку событий и команд.

Классы
-------
- Watchdog: следит за живостью hardware-потоков
- ScudEngine: главный оркестратор, запускающий и координирующий все модули

Методы ScudEngine
------------------
- __init__: инициализировать движок с конфигурацией и таймингами
- start: запустить все hardware-потоки
- stop: остановить все потоки
- is_healthy: проверить состояние потоков
- get_event_queue: получить очередь событий
- get_command_queue: получить очередь команд
- send_command: отправить команду в очередь команд
"""
...
class ScudEngine:
    ...
    def run_lgtu_controller(self) -> None:
        ...
        cache_path = os.path.join(script_dir, "local_access.json")
