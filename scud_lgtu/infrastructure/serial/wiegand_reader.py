"""
Поток считывания карт по протоколу Wiegand (WeigandReader) системы СКУД.

Этот модуль реализует полную замену оригинального wiegand_reader.WeigandReader
с переходом с multiprocessing.Process на threading.Thread. Считыватель передаёт
биты по двум линиям (D0 и D1): импульс LOW на D0 передаёт бит «0», импульс LOW
на D1 передаёт бит «1». Поток блокируется через select на файловом дескрипторе gpiod,
накапливает биты и при достижении нужного числа или истечении таймаута обрабатывает карту.

Классы
-------
- CardData: данные считанной карты
- WeigandReader: поток считывания карт по протоколу Wiegand

Методы WeigandReader
--------------------
- __init__: инициализировать считыватель с пинами D0/D1 и форматом
- start: запустить поток считывания
- stop: остановить поток
- run: главный цикл считывания карт
- _resolve_pin: преобразовать имя пина или offset в (chip_path, line_offset)
"""
...
class WeigandReader:
    ...
    def __init__(
        ...
        # Защита от шума после валидной карты
        self._ignore_until: float = 0.0
        self._ignore_duration: float = ignore_after_valid
    ...
    def _process_bit(self, bit_value: int) -> None:
        ...
        # Игнорировать события после валидной карты (защита от шума)
    ...
    def _process_card(self) -> None:
        ...
        if is_valid:
            self._ignore_until = time.time() + self._ignore_duration
    ...
    def start(self) -> None:
        """
        Запустить фоновый поток чтения Wiegand.

        Если поток уже запущен — игнорируется.
        """
        if hasattr(self, '_thread') and self._thread is not None and self._thread.is_alive():
            logger.warning("[WeigandReader] Поток уже запущен.")
            return

        self.running.set()
        self._thread = threading.Thread(
            target=self.run,
            name=f"WeigandReader-{self.total_bits}",
            daemon=True,
        )
        self._thread.start()
        logger.info("[WeigandReader] Поток запущен.")

    def stop(self, timeout: float = 5.0) -> None:
        """
        Остановить фоновый поток чтения Wiegand.
        ...
        """
        if not hasattr(self, '_thread') or self._thread is None or not self._thread.is_alive():
            logger.info("[WeigandReader] Поток не запущен.")
            return
        ...
        if self._thread.is_alive():
            logger.warning(
                "[WeigandReader] Поток не завершился за %ss.", timeout
            )
        else:
            logger.info("[WeigandReader] Поток остановлен.")
    ...
    def start(
        ...
        ignore_after_valid : float
            Время игнорирования событий после валидной карты.
        ...
        reader = cls(
            ...
            ignore_after_valid=ignore_after_valid,
        )
        t = threading.Thread(
            target=reader.run,
