...
try:
    ...
    from .qr_handler import encode_qr
    from .data_types import ScudCommand, CommandTarget, CommandAction
except ImportError:
    # Для запуска как standalone скрипта
    from engine import ScudEngine
    from access_controller import AccessController
    from config import load as load_config
    from qr_handler import encode_qr
...
class ScudCLI:
    ...
    def view_cache(self) -> None:
        ...
        print(f"Локальный кэш: {cache.count()} записей")
    ...
    def add_identifier(self, identifier: str) -> None:
        ...
        self.controller._cache.add(identifier, True)
    ...
    def remove_identifier(self, identifier: str) -> None:
        ...
        self.controller._cache.remove(identifier)
