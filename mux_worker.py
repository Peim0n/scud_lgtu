"""
Поток опроса мультиплексора (MuxWorker).
...
3. После выставления адреса выдерживается пауза ``addr_settle_s``
   ...
   чтобы не блокировать ShiftRegWorker на всё время ожидания.
...
5. Если состояние хотя бы одного адреса изменилось — результат кладётся
   в ``output_queue``.
6. Доступ к GPIO защищён ``threading.Lock``, общим с ShiftRegWorker,
...
#_DEFAULT_ADDR_SETTLE_S: float = 0.1   # 100 мс
...
class MuxWorker:
    ...
    def _work_mux(self) -> None:
        ...
        захватом внешнего лока** — иначе ShiftRegWorker успеет изменить пины
        ...
        Последовательность для каждого из 2^n адресов:
          1. Захватываем ``self._lock`` (внешний, разделяемый с ShiftRegWorker).
          ...
          3. Спим ``addr_settle_s`` **внутри** внешнего лока.
             ShiftRegWorker заблокирован на это время, но задержка мала (300 мкс)
        ...
        for mask in range(2 ** self._n):
            ...
            # ShiftRegWorker не вмешается в середину последовательности.
            with self._lock:
        ...
        if buf != self._prev_state:
            ...
            try:
                ...
                if not self._overflow_logged:
                    logger.warning("MuxWorker: output_queue переполнена, данные сброшены.")
            ...
            if self._event_queue is not None:
                try:
                    ...
                    if not self._overflow_logged:
                        logger.warning("MuxWorker: event_queue переполнена.")
                        self._overflow_logged = True
